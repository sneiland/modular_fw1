...
function PickAndUploadFile(editor, forImage, caller) {
	...
	if (!filePicker)
	{
		...
		filePicker.addEventListener("change", function () {
				...
				// Upload the file
				if (uploadFile( filePickerEditor, file, id, fileName, filePickerForceLink, data ))
	...
	init : function( editor )
	{
			...
			html = html.replace( /<img src="data:image\/.{3,4};base64,.*?" alt="">/g, function( img )
				{
					...
					var xhr = createXHRupload(editor, id, fileName, false, uploadData);
					if (!xhr)
						return "";
		...
		*/
		...
		editor.ui.addButton && editor.ui.addButton( 'addImage', {
			...
			toolbar: 'insert',
			allowedContent : 'img[!src];span[id](ImagePasteTmpWrapper);',
		...
		var rootDropListener = function(e) {
			...
			if ( data && data.files && data.files.length>0 )
			{
				...
				for( var i=0; i<data.files.length; i++)
				{
					...
					if (!uploadFile( editor, file, id, fileName, false, evdata ))
						continue;
		...
		{
			editor.addFeature( {
				allowedContent: 'img[!src];a[!href];span[id](ImagePasteTmpWrapper);'
			} );
		}
...
function createXHRupload(editor, id, fileName, forceLink, data)
{
	...
	xhr.onload = function() {
		...
		editor.fire('imagepaste.endUpload', { name: fileName, ok: (!!fileUrl) } );
		if (xhr.status!=200)
		{
...
function uploadFile(editor, file, id, fileName, forceLink, data)
{
	data.file = file;
	var xhr = createXHRupload(editor, id, fileName, forceLink, data);
